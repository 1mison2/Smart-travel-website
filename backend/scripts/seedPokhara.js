...
const { locationSeeds, listingSeeds, nearbyRecommendations } = require("../seeds/pokharaSeedData");
const { tripPackageSeeds } = require("../seeds/pokharaTripPackageSeeds");
...
async function upsertLocations() {
  const hubSeed = locationSeeds.find((item) => item.key === "pokhara");
  const childSeeds = locationSeeds.filter((item) => item.key !== "pokhara");
  ...
  const pokharaHub = await Location.findOneAndUpdate(
    {
      name: hubSeed.name,
      district: hubSeed.district,
      province: hubSeed.province,
  ...
  const locationMap = new Map([[hubSeed.key, pokharaHub]]);
    ...
    const location = await Location.findOneAndUpdate(
      ...
      },
      {
        $set: {
          ...
          province: seed.province,
          district: seed.district,
          parentLocationId: pokharaHub._id,
...
async function run() {
  ...
  const pokhara = locationMap.get("pokhara");
  const childCount = Array.from(locationMap.keys()).filter((key) => key !== "pokhara").length;

  console.log("Pokhara seed import completed.");
  console.log(`Creator account: ${creator.email}`);
  console.log(`Destination hub: ${pokhara.name}`);
  console.log(`Child places linked under Pokhara: ${childCount}`);
...
run()
  .catch((error) => {
    console.error("Pokhara seed import failed.");
    console.error(error);
    process.exitCode = 1;
  })
  .finally(async () => {
