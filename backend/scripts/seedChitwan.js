...
const { locationSeeds, listingSeeds, nearbyRecommendations } = require("../seeds/chitwanSeedData");
const { tripPackageSeeds } = require("../seeds/chitwanTripPackageSeeds");
...
async function upsertLocations() {
  const hubSeed = locationSeeds.find((item) => item.key === "chitwan");
  const childSeeds = locationSeeds.filter((item) => item.key !== "chitwan");
  ...
  const chitwanHub = await Location.findOneAndUpdate(
    {
      name: hubSeed.name,
      district: hubSeed.district,
      province: hubSeed.province,
  ...
  const locationMap = new Map([[hubSeed.key, chitwanHub]]);
    ...
    const location = await Location.findOneAndUpdate(
      ...
      },
      {
        $set: {
          ...
          province: seed.province,
          district: seed.district,
          parentLocationId: chitwanHub._id,
...
async function run() {
  ...
  const chitwan = locationMap.get("chitwan");
  const childCount = Array.from(locationMap.keys()).filter((key) => key !== "chitwan").length;

  console.log("Chitwan seed import completed.");
  console.log(`Creator account: ${creator.email}`);
  console.log(`Destination hub: ${chitwan.name}`);
  console.log(`Child places linked under Chitwan: ${childCount}`);
...
run()
  .catch((error) => {
    console.error("Chitwan seed import failed.");
    console.error(error);
    process.exitCode = 1;
  })
  .finally(async () => {
