...
export default function BlogFeedSection({ onNotify }) {
  ...
  const loadBlogs = async () => {
    try {
      ...
      const [publicRes, mineRes] = await Promise.all([
        api.get("/api/posts", { params: { type: "blog" } }),
        api.get("/api/posts", { params: { type: "blog", mine: "true" } }),
      ]);
      const merged = [...(publicRes.data?.posts || []), ...(mineRes.data?.posts || [])].reduce((acc, post) => {
        if (post?._id && !acc.some((item) => item._id === post._id)) acc.push(post);
        return acc;
      }, []);
      setBlogs(merged);
    } catch {
  ...
  const isMine = (blog) => String(blog?.userId?._id || blog?.userId) === String(user?._id || user?.id);
  ...
  const saveEdit = async (blogId) => {
    try {
      await api.put(`/api/posts/${blogId}`, { ...editForm, type: "blog" });
      cancelEdit();
      await loadBlogs();
      onNotify?.({ type: "success", message: "Blog updated and sent for moderation." });
    } catch (err) {
      onNotify?.({ type: "error", message: err?.response?.data?.message || "Failed to update blog." });
    }
  };
  ...
  const deleteBlog = async (blogId) => {
    try {
      ...
      await loadBlogs();
      onNotify?.({ type: "success", message: "Blog deleted." });
    } catch (err) {
      onNotify?.({ type: "error", message: err?.response?.data?.message || "Failed to delete blog." });
    }
  };
            ...
            <div className="p-5">
              {editingId === blog._id ? (
                <div className="grid gap-3 rounded-[24px] border border-slate-200 bg-slate-50 p-4">
                  <input value={editForm.title} onChange={(e) => setEditForm((c) => ({ ...c, title: e.target.value }))} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none" />
                  <input value={editForm.destination} onChange={(e) => setEditForm((c) => ({ ...c, destination: e.target.value }))} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none" />
                  <input value={editForm.tags} onChange={(e) => setEditForm((c) => ({ ...c, tags: e.target.value }))} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none" />
                  <textarea value={editForm.content} onChange={(e) => setEditForm((c) => ({ ...c, content: e.target.value }))} rows={4} className="rounded-[22px] border border-slate-200 bg-white px-4 py-3 outline-none" />
                  <div className="flex flex-wrap gap-2">
                    <button type="button" onClick={() => saveEdit(blog._id)} className="rounded-full bg-emerald-500 px-4 py-2 text-sm font-semibold text-white">Save changes</button>
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {isMine(blog) && editingId !== blog._id ? (
                  <button type="button" onClick={() => startEdit(blog)} className="inline-flex items-center gap-2 rounded-full bg-slate-900 px-4 py-2 text-sm font-semibold text-white">
                    Edit
                  </button>
                ) : null}
                {isMine(blog) && editingId !== blog._id ? (
                  <button type="button" onClick={() => deleteBlog(blog._id)} className="inline-flex items-center gap-2 rounded-full bg-red-50 px-4 py-2 text-sm font-semibold text-red-700">
                    Delete
                  </button>
                ) : null}
